...
import { AccidentProvider } from './store/AccidentContext';
...
// Lazy-loaded: the accident wizard is a large component tree (9 steps) only
// needed once the user actually activates Accident Mode.
const AccidentsPage = lazy(() => import('./pages/AccidentsPage'));
const AccidentWizardPage = lazy(() => import('./pages/accident/AccidentWizardPage'));

const lazyFallback = <div className="content" style={{ paddingTop: 48, textAlign: 'center', color: 'var(--purple-200)' }}>טוען…</div>;
            ...
            <AccidentProvider>
              <div className="app">
                ...
                <Routes>
                  ...
                  <Route
                    path="/export"
                    element={<Suspense fallback={lazyFallback}><ExportPage /></Suspense>}
                  />
                  <Route
                    path="/accidents"
                    element={<Suspense fallback={lazyFallback}><AccidentsPage /></Suspense>}
                  />
                  <Route
                    path="/accident/:id"
                    element={<Suspense fallback={lazyFallback}><AccidentWizardPage /></Suspense>}
                  />
